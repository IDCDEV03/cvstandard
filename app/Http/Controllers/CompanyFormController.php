...
class CompanyFormController extends Controller
{
    ...
    public function FormStore1(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('company.form.create2', ['id' => $form_id, 'order' => '1'])
    ...
    public function FormNew_Step2($id, $order)
    {
        ...
        return view('pages.company.form_create_2', ['id' => $id, 'order' => $order], compact('chk_cates'));
    }

    public function FormStep2_Store(Request $request, $id)
    {
        ...
        return redirect()->route('company.form.create3', ['id' => $id])->with('success', 'บันทึกหมวดหมู่เรียบร้อยแล้ว');
    }
    ...
    public function FormNew_Step3($id)
    {
        ...
        return view('pages.company.form_create_3', ['id' => $id], compact('data', 'form_name'));
    }

    public function categories_list($cates_id)
    {
        ...
        return view('pages.company.form_categories_list', ['cates_id' => $cates_id], compact('cates_data', 'item_data'));
    }
    ...
    public function item_create($id)
    {
        ...
        return view('pages.company.form_item_create', ['id' => $id], compact('cates_data'));
    }
    ...
    public function item_store(Request $request)
    {
        ...
        try {
            ...
            // 5. Batch Insert (ทำงานเร็วและใช้ทรัพยากร Database น้อยลงมาก)
            if (!empty($insertData)) {
                DB::table('check_items')->insert($insertData);
            }

            DB::commit();
            return redirect()->route('company.form.categories_list', ['cates_id' => $request->cate_id])
                ->with('success', 'บันทึกข้อตรวจเรียบร้อยแล้ว');
        } catch (\Exception $e) {
            // 6. Rollback ข้อมูลใน Database
            DB::rollBack();

            // 7. ลบไฟล์ที่ถูกอัปโหลดไปแล้วในรอบนี้ (ป้องกันไฟล์ขยะค้างใน Server)
    ...
    public function backToCreate3($id)
    {
        session()->forget([
            'item_data',
            'current_step_data',
        ...
        return redirect()->route('company.form.create3', ['id' => $id]);
    }
}
